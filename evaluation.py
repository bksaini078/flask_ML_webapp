...
VAT=load_model ( 'models/VAT_regularization')
Psuedo_Label=load_model ( 'models/Psuedo_Label')
...
def model_prediction(article):
    ...
    # y_vat= (VAT.predict(article_seq))
    # predict_df['VAT_regularization']= np.argmax( y_vat, axis=1 )
    y_p= Psuedo_Label.predict(article_seq)
    predict_df['Psuedo_label'] = bool (np.argmax( y_p, axis=1 ))
    embedding_matrix = load_emb_matrix ( meta_path+'/emb_matrix.npy' )
    # test_feature, test_label = next ( iter ( test_dataset ) )
    test_emb = embedding( article_seq, embedding_matrix )
    y_vat = (VAT.predict ( test_emb ))
    predict_df['VAT regularization'] = bool ( np.argmax ( y_vat, axis=1 ) )
...
def model_prediction_df(df):
    ...
    y_p = Psuedo_Label.predict ( x_test_seq )
    predict_df['Psuedo_label'] =  ( np.argmax ( y_p, axis=1 ) ).astype(bool)
    embedding_matrix = load_emb_matrix ( meta_path + '/emb_matrix.npy' )
    # test_feature, test_label = next ( iter ( test_dataset ) )
    test_emb = embedding ( x_test_seq, embedding_matrix )
    y_vat = (VAT.predict ( test_emb ))
    predict_df['VAT regularization'] =  ( np.argmax ( y_vat, axis=1 ) ).astype(bool)
    predict_df.replace( {False : 'Fake'} )
    return predict_df
